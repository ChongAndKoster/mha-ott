...
def rational_response_request_theme_new_1_emotion(situation, thought, cognitive_distortion_category, curr_theme, emotion):
	...
	if curr_theme in PROMPTS_THEME_NEW:
		...
		if curr_safety_filter_thought:
			curr_reframing_prompt = 'Write a rational response to the following distorted thoughts. Empathize based on the provided emotion whenever possible. Do not use words related to suicidal ideation or self-harm such as "die", "suicide", or "cut":\n\n' + '\n\n'.join(curr_prompt_example_li)
			curr_prompt_str = 'THEME_PROMPT_1'


		else:
			curr_reframing_prompt = 'Write a rational response to the following distorted thoughts. Empathize based on the provided emotion whenever possible. \n\n' + '\n\n'.join(curr_prompt_example_li)
			curr_prompt_str = 'THEME_PROMPT_1'
...
def rational_response_request_theme_new_2_emotion(situation, thought, cognitive_distortion_category, curr_theme, emotion):
...
def rational_response_request_theme_new_3_emotion(situation, thought, cognitive_distortion_category, curr_theme, emotion):
...
df = pd.read_csv('/projects/bdata/talklife/dssg/ashish/Dataset/Cognitive-Distortions/changing_thoughts_tool/changing-thoughts-records-Oct-11-with-emotions.csv')
...
for ii in tqdm(range(50)):
	...
	emotion = df[df['thought_record_id'] == thought_record_id]['emotion'].values[0]
	...
	emotion_1 = rational_response_request_theme_new_1_emotion(situation, thought, cognitive_distortion_category, curr_theme, emotion)
	emotion_2 = rational_response_request_theme_new_2_emotion(situation, thought, cognitive_distortion_category, curr_theme, emotion)
	emotion_3 = rational_response_request_theme_new_3_emotion(situation, thought, cognitive_distortion_category, curr_theme, emotion)

	gpt3_reframes = ';'.join([gpt3_1, gpt3_2, gpt3_3])
	emotion_reframes = ';'.join([emotion_1, emotion_2, emotion_3])

	output_li.append([thought_record_id, situation, thought, cognitive_distortion_category, emotion, curr_theme, gpt3_reframes, emotion_reframes])


	output_df = pd.DataFrame(output_li, columns=['thought_record_id', 'situation', 'thought', 'cognitive_distortion_category', 'emotion', 'curr_theme', 'non_emotion_reframes', 'emotion_reframes'])

	output_df.to_csv('/projects/bdata/talklife/dssg/ashish/Dataset/Cognitive-Distortions/changing_thoughts_tool/changing-thoughts-records-Oct-11-sample-emotions.csv', index=False)
